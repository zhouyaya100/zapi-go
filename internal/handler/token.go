...
func HandleCreateToken(c *gin.Context) {
	...
	if !middleware.GetAuth(c).IsAdminToken && req.Models != "" && tu.AllowedModels != "" {
		...
		for m := range requested {
			found := false
			for _, am := range core.SplitComma(tu.AllowedModels) { if am == m { found = true; break } }
			if !found { c.JSON(403, gin.H{"error": gin.H{"message": fmt.Sprintf("\u65e0\u6743\u4f7f\u7528\u6a21\u578b: %s", m)}}); return }
		}
	}
		...
		if !middleware.GetAuth(c).IsAdminToken {
			...
			if v != "" && u.AllowedModels != "" {
				for _, m := range core.SplitComma(v) {
					...
					if !found { c.JSON(403, gin.H{"error": gin.H{"message": fmt.Sprintf("\u65e0\u6743\u4f7f\u7528\u90e8\u5206\u6a21\u578b")}}); return }
				}
			}
		}
		tk.Models = v
