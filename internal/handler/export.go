...
func exportGrouped(c *gin.Context, groupBy string, fmt_ string) {
	...
	case "day":
		...
		var rows []row
		qry.Select(core.TzDateExpr()+" as period, count(*) as requests, sum(case when success then 1 else 0 end) as success, coalesce(sum(prompt_tokens),0) as prompt_tokens, coalesce(sum(completion_tokens),0) as completion_tokens, coalesce(avg(latency_ms),0) as avg_latency_ms").Group(core.TzDateExpr()).Order("period desc").Scan(&rows)
	...
	case "user":
		type row struct{ UserID uint; Requests int64; Success int64; PromptTokens int64; CompletionTokens int64; AvgLatencyMs float64 }
		var rows []row
		qry.Select("user_id, count(*) as requests, sum(case when success then 1 else 0 end) as success, coalesce(sum(prompt_tokens),0) as prompt_tokens, coalesce(sum(completion_tokens),0) as completion_tokens, coalesce(avg(latency_ms),0) as avg_latency_ms").Group("user_id").Order("requests desc").Scan(&rows)
	...
	case "model":
		...
		var rows []row
		qry.Select("model, count(*) as requests, sum(case when success then 1 else 0 end) as success, coalesce(sum(prompt_tokens),0) as prompt_tokens, coalesce(sum(completion_tokens),0) as completion_tokens, coalesce(avg(latency_ms),0) as avg_latency_ms").Group("model").Order("requests desc").Scan(&rows)
		items := make([]map[string]interface{}, len(rows))
		for i, r := range rows {
			fail := r.Requests - r.Success; sr := 0.0; if r.Requests > 0 { sr = float64(r.Success) / float64(r.Requests) * 100 }
		...
		if fmt_ == "xlsx" { writeXLSXGrouped(c, items, groupBy, nowStr) } else { writeCSVGrouped(c, items, groupBy, nowStr) }
	case "channel":
		type row struct{ ChannelID uint; Requests int64; Success int64; PromptTokens int64; CompletionTokens int64; AvgLatencyMs float64 }
		var rows []row
		qry.Select("channel_id, count(*) as requests, sum(case when success then 1 else 0 end) as success, coalesce(sum(prompt_tokens),0) as prompt_tokens, coalesce(sum(completion_tokens),0) as completion_tokens, coalesce(avg(latency_ms),0) as avg_latency_ms").Group("channel_id").Order("requests desc").Scan(&rows)
		...
		if fmt_ == "xlsx" { writeXLSXGrouped(c, items, groupBy, nowStr) } else { writeCSVGrouped(c, items, groupBy, nowStr) }
	case "summary":
		type row struct{ UserID uint; Model string; ChannelID uint; Requests int64; Success int64; PromptTokens int64; CompletionTokens int64; AvgLatencyMs float64 }
		var rows []row
		qry.Select("user_id, model, channel_id, count(*) as requests, sum(case when success then 1 else 0 end) as success, coalesce(sum(prompt_tokens),0) as prompt_tokens, coalesce(sum(completion_tokens),0) as completion_tokens, coalesce(avg(latency_ms),0) as avg_latency_ms").Group("user_id, model, channel_id").Order("requests desc").Scan(&rows)
		um := make(map[uint]string); var users []model.User; model.DB.Select("id, username").Find(&users); for _, u := range users { um[u.ID] = u.Username }
		cm := make(map[uint]string); var chs []model.Channel; model.DB.Select("id, name").Find(&chs); for _, ch := range chs { cm[ch.ID] = ch.Name }
		items := make([]map[string]interface{}, len(rows))
		for i, r := range rows {
...
func writeCSVDetail(c *gin.Context, rows []map[string]interface{}, nowStr string) {
	...
	w := csv.NewWriter(&buf)
	w.Write([]string{"ID", "用户名", "令牌名", "渠道名", "模型", "流式", "输入Token", "输出Token", "总Token", "延迟(ms)", "成功", "错误信息", "客户端IP", "时间"})
	for _, r := range rows {
...
func writeCSVGrouped(c *gin.Context, items []map[string]interface{}, groupBy string, nowStr string) {
	...
	w.Write([]string{labels[groupBy], "请求数", "成功数", "失败数", "成功率", "输入Token", "输出Token", "总Token", "平均延迟(ms)"})
	for _, r := range items {
		w.Write([]string{fmt.Sprintf("%v", r["key"]), fmt.Sprintf("%v", r["requests"]), fmt.Sprintf("%v", r["success"]), fmt.Sprintf("%v", r["fail"]), fmt.Sprintf("%v", r["success_rate"]), fmt.Sprintf("%v", r["prompt_tokens"]), fmt.Sprintf("%v", r["completion_tokens"]), fmt.Sprintf("%v", r["total_tokens"]), fmt.Sprintf("%v", r["avg_latency_ms"])})
	}
	w.Flush()
	filename := url.QueryEscape(fmt.Sprintf("用量报表_%s.csv", nowStr))
	c.Header("Content-Disposition", fmt.Sprintf("attachment; filename*=UTF-8''%s", filename))
...
func writeCSVSummary(c *gin.Context, items []map[string]interface{}, nowStr string) {
	...
	buf.Write([]byte{0xEF, 0xBB, 0xBF})
	w := csv.NewWriter(&buf)
	w.Write([]string{"用户", "模型", "渠道", "请求数", "成功数", "失败数", "成功率", "输入Token", "输出Token", "总Token", "平均延迟(ms)"})
	for _, r := range items {
...
func writeXLSXDetail(c *gin.Context, rows []map[string]interface{}, nowStr string) {
	...
	for i, r := range rows {
		...
		f.SetCellValue(sheet, fmt.Sprintf("I%d", row), r["total_tokens"])
		f.SetCellValue(sheet, fmt.Sprintf("J%d", row), r["latency_ms"])
		f.SetCellValue(sheet, fmt.Sprintf("K%d", row), r["success"])
		f.SetCellValue(sheet, fmt.Sprintf("L%d", row), r["error_msg"])
		f.SetCellValue(sheet, fmt.Sprintf("M%d", row), r["client_ip"])
		f.SetCellValue(sheet, fmt.Sprintf("N%d", row), r["created_at"])
	}
...
func writeXLSXGrouped(c *gin.Context, items []map[string]interface{}, groupBy string, nowStr string) {
	...
	for i, r := range items {
		...
		f.SetCellValue(sheet, fmt.Sprintf("H%d", row), r["total_tokens"])
		f.SetCellValue(sheet, fmt.Sprintf("I%d", row), r["avg_latency_ms"])
	}
	buf, _ := f.WriteToBuffer()
	filename := url.QueryEscape(fmt.Sprintf("用量报表_%s.xlsx", nowStr))
	c.Header("Content-Disposition", fmt.Sprintf("attachment; filename*=UTF-8''%s", filename))
...
func writeXLSXSummary(c *gin.Context, items []map[string]interface{}, nowStr string) {
	...
	for i, r := range items {
		...
		f.SetCellValue(sheet, fmt.Sprintf("J%d", row), r["total_tokens"])
		f.SetCellValue(sheet, fmt.Sprintf("K%d", row), r["avg_latency_ms"])
	}
	buf, _ := f.WriteToBuffer()
	filename := url.QueryEscape(fmt.Sprintf("用量报表_%s.xlsx", nowStr))
	c.Header("Content-Disposition", fmt.Sprintf("attachment; filename*=UTF-8''%s", filename))
