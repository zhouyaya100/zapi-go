...
func HandleStats(c *gin.Context) {
	...
	var avgLat float64
	model.DB.Model(&model.Log{}).Select("count(id), sum(case when success then 1 else 0 end), coalesce(sum(prompt_tokens),0), coalesce(sum(completion_tokens),0), coalesce(avg(latency_ms),0)").Row().Scan(&tr, &sr2, &tp, &tcm, &avgLat)
...
func HandleUsageStats(c *gin.Context) {
	...
	var tr, sr2, tp, tcm int64
	var avgLat float64
	q.Select("count(id), sum(case when success then 1 else 0 end), coalesce(sum(prompt_tokens),0), coalesce(sum(completion_tokens),0), coalesce(avg(latency_ms),0)").Row().Scan(&tr, &sr2, &tp, &tcm, &avgLat)
	summary := gin.H{
		"total_requests": tr, "success_requests": sr2,
		"total_prompt_tokens": core.SafeInt(tp), "total_completion_tokens": core.SafeInt(tcm),
		"total_tokens": core.SafeInt(tp+tcm), "avg_latency_ms": int(avgLat),
	...
	case "model":
		...
		var rows []row
		q.Select("model, count(*) as requests, sum(case when success then 1 else 0 end) as success, coalesce(sum(prompt_tokens),0) as prompt_tokens, coalesce(sum(completion_tokens),0) as completion_tokens, coalesce(avg(latency_ms),0) as avg_latency_ms").Group("model").Order("requests desc").Scan(&rows)
		items := make([]gin.H, len(rows))
		for i, r := range rows {
			items[i] = gin.H{"key": r.Model, "requests": r.Requests, "success": r.Success, "prompt_tokens": core.SafeInt(r.PromptTokens), "completion_tokens": core.SafeInt(r.CompletionTokens), "fail": r.Requests-r.Success, "success_rate": fmt.Sprintf("%.1f%%", float64(r.Success)/float64(max(r.Requests,1))*100), "total_tokens": core.SafeInt(r.PromptTokens+r.CompletionTokens), "avg_latency_ms": int(r.AvgLatencyMs)}
		}
		c.JSON(200, gin.H{"summary": summary, "items": items, "total": len(rows), "page": page, "page_size": pageSize})
	...
	case "user":
		...
		var rows []row
		q.Select("user_id, count(*) as requests, sum(case when success then 1 else 0 end) as success, coalesce(sum(prompt_tokens),0) as prompt_tokens, coalesce(sum(completion_tokens),0) as completion_tokens, coalesce(avg(latency_ms),0) as avg_latency_ms").Group("user_id").Order("requests desc").Scan(&rows)
	...
	case "channel":
		...
		var rows []row
		q.Select("channel_id, count(*) as requests, sum(case when success then 1 else 0 end) as success, coalesce(sum(prompt_tokens),0) as prompt_tokens, coalesce(sum(completion_tokens),0) as completion_tokens, coalesce(avg(latency_ms),0) as avg_latency_ms").Group("channel_id").Order("requests desc").Scan(&rows)
