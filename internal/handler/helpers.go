...
func HandleVersion(c *gin.Context) {
	c.JSON(200, gin.H{"version": "4.0.2"})
}
...
func HandleMyModels(c *gin.Context) {
	...
	if u.ID == model.SuperAdminID || u.Role == "admin" {
		// Admin/operator sees all models from pool
		models := routing.Pool.GetAllEnabledModels()
		c.JSON(200, gin.H{"models": models})
		return
	}
	models := core.GetGroupAuthedModels(gn, u.AllowedModels, routing.Pool)
	c.JSON(200, gin.H{"models": models})
}
...
func HandleMyDashboard(c *gin.Context) {
	...
	authedModels := []string{}
	if rl.GroupName != "" {
		authedModels = core.GetGroupAuthedModels(rl.GroupName, u.AllowedModels, routing.Pool)
	}
